...
void MCMC_state::update_suffstats() 
{
	...
	for (size_t i = 0; i < cls_assgn.size(); i++)
	{
		...
		double tmp = (square(rho3) - 1) * square(beta_1) + (square(rho2) - 1) * square(beta_2);
		tmp = tmp + (square(rho1) - 1) * square(beta_3) + 2 * (rho1 - rho2 * rho3) * beta_1 * beta_2;
		tmp = tmp + 2 * (rho2 - rho1 * rho3) * beta_1 * beta_3 + 2 * (rho3 - rho2 * rho1) * beta_3 * beta_2;
...
void MCMC_state::sample_sigma2() 
{
    ...
    for (size_t i = 1; i < num_cluster; i++) 
	{
		...
		double b = 1.0 / (sumsq[i] / (2.0 * s0) + para.b0k);
...
void MCMC_state::calc_b(size_t j, const mcmc_data &dat1, const mcmc_data &dat2, const mcmc_data &dat3, \
...
void MCMC_state::update_p() 
{
    ...
    double sum = std::accumulate(p.begin() + 1, p.end() - 1, 0.0);
    if (1 - sum > 0) 
	{
		p[M] = 1 - sum;
    }
...
void MCMC_state::compute_h2(const mcmc_data &dat1, const mcmc_data &dat2, const mcmc_data &dat3) 
{
    ...
    double h21_tmp, h22_tmp = 0, h23_tmp = 0;
    h22 = 0; h21 = 0, h23 = 0;
...
void MCMC_state::sample_assignment(size_t j, const mcmc_data &dat1, const mcmc_data &dat2, const mcmc_data &dat3,\
									const ldmat_data &ldmat_dat1, const ldmat_data &ldmat_dat2, const ldmat_data &ldmat_dat3) 
{
	size_t start_i = dat1.boundary[j].first;
    size_t end_i = dat1.boundary[j].second;
	...
	for (size_t i = 0; i < blk_size; i++) 
	{
		...
		{
			double denomk = s0 * cluster_var[k];

			double c1 = (rho1 - rho2 * rho3) / denomk;
			double c2 = (rho2 - rho1 * rho3) / denomk;
			double c3 = (rho3 - rho1 * rho2) / denomk;

			double a1 = N1 * square(eta) * B1jj[i] + (1 - square(rho3)) / denomk;
			double a2 = N2 * square(eta) * B2jj[i] + (1 - square(rho2)) / denomk;
			double a3 = N3 * square(eta) * B3jj[i] + (1 - square(rho1)) / denomk;
	    	...
	    	prob[i][k] = 0.5 * (L11 * square(Nb1) + L22 * square(Nb2) + L33 * square(Nb3) + \
				2 * L12 * Nb1 * Nb2 + 2 * L13 * Nb1 * Nb3 + 2 * L23 * Nb2 * Nb3);
	...
	for (size_t i = 0; i < blk_size; i++) 
	{
		float max_elem = *std::max_element(&prob[i][0], &prob[i][num_cluster - 1]); // why we minus one GHL
		float log_exp_sum = 0;
...
void MCMC_state::sample_beta(size_t j, const mcmc_data &dat1, const mcmc_data &dat2, const mcmc_data &dat3, \
							ldmat_data &ldmat_dat1, ldmat_data &ldmat_dat2, ldmat_data &ldmat_dat3) 
{
    size_t start_i = dat1.boundary[j].first;
    size_t end_i = dat1.boundary[j].second;
	...
	double F11 = 1 - square(rho3);
	double F12 = rho2 * rho3 - rho1;
	double F13 = rho1 * rho3 - rho2;
	double F22 = 1 - square(rho2);
	double F23 = rho1 * rho2 - rho3;
	double F33 = 1 - square(rho1);
	...
	for (size_t i = 0; i < B_size; i++)
	{
		...
		for (size_t k = 0; k < B_size; k++)
		{
			if (i < causal_size)
			{
				if (k == i) value = F11 / (s0 * cluster_var[cls_assgn[causal_list[i]]]);
				else if (k == causal_size + i) value = F12 / (s0 * cluster_var[cls_assgn[causal_list[i]]]);
				else if (k == 2 * causal_size + i) value = F13 / (s0 * cluster_var[cls_assgn[causal_list[i]]]);
				else value = 0;
				if (k < causal_size) 
				{
					value = value + C1 * ldmat_dat1.B[j] -> data[ldmat_dat1.B[j]->tda * (causal_list[i] - start_i) + causal_list[k] - start_i];
				}
			...
			if (i >= causal_size && i < 2 * causal_size) 
			{
				if (k == i) value = F22 / (s0 * cluster_var[cls_assgn[causal_list[i - causal_size]]]);
				else if (k == (i - causal_size)) value = F12 / (s0 * cluster_var[cls_assgn[causal_list[i - causal_size]]]);
				else if (k == (causal_size + i)) value = F23 / (s0 * cluster_var[cls_assgn[causal_list[i - causal_size]]]);
				else value = 0;
			...
			if (i >= 2 * causal_size) 
			{
				if (k == i) value = F33 / (s0 * cluster_var[cls_assgn[causal_list[i - 2 * causal_size]]]);
				else if (k == (i - causal_size)) value = F23 / (s0 * cluster_var[cls_assgn[causal_list[i - 2 * causal_size]]]);
				else if (k == (i - 2 * causal_size)) value = F13 / (s0 * cluster_var[cls_assgn[causal_list[i - 2 * causal_size]]]);
				else value = 0;
				if (k > 2 * causal_size)   
				{
