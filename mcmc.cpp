...
void MCMC_state::update_suffstats(size_t m) 
{
...
void MCMC_state::calc_b(size_t j, const mcmc_data &dat1,  \
...
void MCMC_state::sample_V(size_t m) 
{
	vector<double> a(M - 1);
	a[M - 2] = suff_stats[M];
	for (int k = M - 3; k >= 0; k--) 
...
void MCMC_state::sample_alpha(size_t m) 
{
	double sum = 0;
...
void MCMC_state::update_p(size_t m) 
{
	p[0] = theta;
...
void MCMC_state::sample_assignment(size_t j, size_t m, const mcmc_data &dat1,\
									const ldmat_data &ldmat_dat1, const ldmat_data &ldmat_dat2, const ldmat_data &ldmat_dat3) 
{
	size_t start_i = dat1.boundary[j].first;
    size_t end_i = dat1.boundary[j].second;
...
void MCMC_state::sample_beta(size_t j, size_t m, const mcmc_data &dat1, \
		  ...
		  int iter, int burn, int thin, unsigned n_threads, int opt_llk, double c1, double c2, double c3)
{
	...
	for (int j = 1; j < iter + 1; j++)
	{
		...
		{
			state.sample_assignment(i, j, dat1, ldmat_dat1, ldmat_dat2, ldmat_dat3);
		}
		state.update_suffstats(j);
		state.sample_theta();
		state.sample_V(j);

		state.update_p(j);
		state.sample_alpha(j);
		for (size_t i = 0; i < bd_size; i++) 
		{
	    	state.sample_beta(i, j, dat1, ldmat_dat1, ldmat_dat2, ldmat_dat3);
		}
...
// Delete the round paramater for debugging GHL